...
@property(nonatomic, retain) BluetoothPrintStreamHandler *readStreamHandler;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  FlutterEventChannel *stateChannel =
      [FlutterEventChannel eventChannelWithName:NAMESPACE @"/state"
                                binaryMessenger:[registrar messenger]];
  FlutterPosPrinterPlatformPlugin *instance =
      [[FlutterPosPrinterPlatformPlugin alloc] init];
  ...
  // READ STREAM
  FlutterEventChannel *readChannel = [FlutterEventChannel
      eventChannelWithName:@"flutter_pos_printer_platform/read_stream"
           binaryMessenger:[registrar messenger]];
  BluetoothPrintStreamHandler *readStreamHandler =
      [[BluetoothPrintStreamHandler alloc] init];
  [readChannel setStreamHandler:readStreamHandler];
  instance.readStreamHandler = readStreamHandler;
  ...
  if ([@"state" isEqualToString:call.method]) {
    ...
    @try {
      ...
      CBPeripheral *peripheral =
          [_scannedPeripherals objectForKey:[device objectForKey:@"address"]];
    ...
    @try {
      ...
      for (int i = 0; i < len; ++i) {
        //               NSLog(@"** ind_%d (d): %@, %d", i, bytes[i], [bytes[i]
        //               charValue]);
      ...
      [Manager write:data2
          receCallBack:^(NSData *_Nullable data) {
            if (self.readStreamHandler.sink != nil && data != nil) {
              // Convert NSData to FlutterStandardTypedData or
              // NSArray<NSNumber*> FlutterEventChannel expects simpler types
              // usually, but TypedData is efficient. Actually, let's just send
              // NSArray<NSNumber*>* for consistency with Android approach (int
              // list) Or FlutterStandardTypedData if we want raw bytes. Android
              // sends List<Integer>.
              ...
              self.readStreamHandler.sink(byteArr);
            }
          }];
      result(nil);
    } @catch (FlutterError *e) {
...
- (void)startScan {
  ...
  [Manager
      scanForPeripheralsWithServices:nil
                             options:nil
                            discover:^(CBPeripheral *_Nullable peripheral,
                              ...
                              if (peripheral.name != nil) {
                                ...
                                [self.scannedPeripherals
                                    setObject:peripheral
                                       forKey:[[peripheral identifier]
                                                  UUIDString]];

                                NSDictionary *device = [NSDictionary
                                    dictionaryWithObjectsAndKeys:
                                        peripheral.identifier.UUIDString,
                                        @"address", peripheral.name, @"name",
                                        nil, @"type", nil];
...
#endif
    NSDictionary *dict =
        [NSDictionary dictionaryWithObjectsAndKeys:ret, @"id", nil];
